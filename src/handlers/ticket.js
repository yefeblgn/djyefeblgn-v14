...
async function parseTicketDetails(channel) {
  ...
  const userId = split[1];
    const catName = split[2] || "Varsayılan";
...
async function closeTicket(channel, closedBy, reason) {
  ...
  try {
    ...
    const logsUrl = await postToBin(content, `Destek Talebi Kayıtları => ${channel.name}`);
    ...
    if (logsUrl) {
      components.push(
        new ActionRowBuilder().addComponents(
          new ButtonBuilder().setLabel("Kaydını Al").setURL(logsUrl.short).setStyle(ButtonStyle.Link)
        )
      );
    }
    ...
    const embed = new EmbedBuilder().setAuthor({ name: "Destek Talebi Kapatıldı." }).setColor(TICKET.CLOSE_EMBED);
    const fields = [];
    ...
    fields.push(
      {
        name: "Açan:",
            value: ticketDetails.user ? ticketDetails.user.tag : "Bilinmeyen",
        inline: true,
      },
      {
        name: "Kapatan:",
          value: closedBy ? closedBy.tag : "Bilinmeyen",
        inline: true,
      }
    );
