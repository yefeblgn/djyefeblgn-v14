...
async function performAutomod(message, settings) {
  ...
  if (mentions.members.size > automod.max_mentions) {
      fields.push({ name: "Bahsetmeler", value: `${mentions.members.size}/${automod.max_mentions}`, inline: true });
  ...
  if (mentions.roles.size > automod.max_role_mentions) {
      fields.push({ name: "Rol Bahsetmeleri", value: `${mentions.roles.size}/${automod.max_role_mentions}`, inline: true });
  ...
  if (automod.anti_massmention > 0) {
    ...
    if (mentions.everyone) {
      fields.push({ name: "@everyone Bashsetmeleri", value: "✓", inline: true });
      strikesTotal += 1;
    }
    ...
    if (mentions.users.size + mentions.roles.size > automod.anti_massmention) {
      fields.push({
          name: "Kullanıcı/Rol Bahsetmeleri",
  ...
  if (automod.max_lines > 0) {
    ...
    if (count > automod.max_lines) {
        fields.push({ name: "Yeni Hatlar", value: `${count}/${automod.max_lines}`, inline: true });
      shouldDelete = true;
  ...
  if (automod.anti_attachments) {
    if (message.attachments.size > 0) {
        fields.push({ name: "Ekler Bulundu", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (automod.anti_links) {
    if (containsLink(content)) {
        fields.push({ name: "Bağlantılar Bulundu", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
      ...
      if (antispamCache.has(key)) {
        ...
        if (
          ...
          Date.now() - antispamInfo.timestamp < MESSAGE_SPAM_THRESHOLD
            ...
            fields.push({ name: "AntiSpam Tespiti", value: "✓", inline: true });
          shouldDelete = true;
          strikesTotal += 1;
        }
      } else {
  ...
  if (!automod.anti_links && automod.anti_invites) {
    if (containsDiscordInvite(content)) {
        fields.push({ name: "Sunucu Davetleri", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (shouldDelete && message.deletable) {
    message
      .delete()
        .then(() => channel.safeSend("> Otomatik Moderasyon! Mesaj silindi", 5))
      .catch(() => {});
  }
  ...
  if (strikesTotal > 0) {
    ...
    if (logChannel) {
      const logEmbed = new EmbedBuilder()
        .setAuthor({ name: "Otomatik Moderasyon" })
        .setThumbnail(author.displayAvatarURL())
        .setColor(AUTOMOD.LOG_EMBED)
        .addFields(fields)
        .setDescription(`**Kanal:** ${channel.toString()}\n**İçerik:**\n${content}`)
        .setFooter({
          text: `${author.tag} | ${author.id} Tarafından`,
    ...
    const strikeEmbed = new EmbedBuilder()
      ...
      .addFields(fields)
      .setDescription(
        `${strikesTotal} adet uyarı aldınız.!\n\n` +
          `**Sunucu:** ${guild.name}\n` +
          `**Toplam Uyarılar:** ${memberDb.strikes} / ${automod.strikes}`
      );
    ...
    if (memberDb.strikes >= automod.strikes) {
      ...
      // Add Moderation Action
        await addModAction(guild.members.me, member, "Otomatik Moderasyon: Maksimum ihtar alındı", automod.action).catch(() => {});
    }
