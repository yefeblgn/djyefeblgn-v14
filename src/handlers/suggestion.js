...
async function approveSuggestion(member, channel, messageId, reason) {
  ...
  const doc = await findSuggestion(guild.id, messageId);
    ...
    if (doc.status === "APPROVED") return "Öneri zaten onaylandı";
  ...
  const approvedEmbed = new EmbedBuilder()
    ...
    .setFooter({ text: `Onaylayan: ${member.user.tag}`, iconURL: member.displayAvatarURL() })
    .setTimestamp();

  const fields = [];

  // add stats if it doesn't exist
    const statsField = message.embeds[0].fields.find((field) => field.name === "İstatistikler");
  if (!statsField) {
    const [upVotes, downVotes] = getStats(message);
    doc.stats.upvotes = upVotes;
    doc.stats.downvotes = downVotes;
      fields.push({ name: "İstatistikler", value: getVotesMessage(upVotes, downVotes) });
  } else {
...
async function rejectSuggestion(member, channel, messageId, reason) {
  ...
  const rejectedEmbed = new EmbedBuilder()
    ...
    .setFooter({ text: `Reddeden: ${member.user.tag}`, iconURL: member.displayAvatarURL() })
    .setTimestamp();
...
async function deleteSuggestion(member, channel, messageId, reason) {
  ...
  try {
    ...
    guild.client.logger.error("deleteSuggestion", ex);
      return "Öneri silinemedi! Lütfen manuel olarak silin";
  }
}
