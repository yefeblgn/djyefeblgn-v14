    ...
    try {
      ...
      for (const config of settings.counters) {
        ...
        if (config.counter_type.toUpperCase() === "BOTS") channelName = `${config.name} : ${bots}`;
          ...
          if (vc.manageable) vc.setName(channelName).catch((err) => vc.client.logger.log("Ad hatası ayarla: ", err));
      }
    } catch (ex) {
        client.logger.error(`GuildId için sayaç kanalları güncellenirken hata oluştu: ${guildId}`, ex);
    } finally {
