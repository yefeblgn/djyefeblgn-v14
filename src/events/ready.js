...
module.exports = async (client) => {
  client.logger.success(`Giriş Yapıldı: ${client.user.tag}! (${client.user.id})`);

  // Initialize Music Manager
  if (client.config.MUSIC.ENABLED) {
    client.musicManager.connect(client.user.id);
      client.logger.success("Müzik Yöneticisi başlatıldı");
  }

  // Initialize Giveaways Manager
  if (client.config.GIVEAWAYS.ENABLED) {
    client.logger.log("Çekilişler Başlatılıyor...");
      client.giveawaysManager._init().then((_) => client.logger.success("Çekiliş Yöneticisi başlatıldı"));
  }
