...
const logModeration = async (issuer, target, reason, type, data = {}) => {
  ...
  switch (type.toUpperCase()) {
    case "PURGE":
      embed.setAuthor({ name: `Moderasyon - ${type}` });
      fields.push(
        { name: "Silme Türü", value: data.purgeType, inline: true },
        { name: "Adet", value: data.deletedCount.toString(), inline: true },
        { name: "Kanal", value: `#${data.channel.name} [${data.channel.id}]`, inline: false }
      );
      break;
  ...
  if (type.toUpperCase() !== "PURGE") {
    embed.setAuthor({ name: `Moderasyon - ${type}` }).setThumbnail(target.displayAvatarURL());

    if (target instanceof GuildMember) {
        fields.push({ name: "Üye", value: `${target.displayName} [${target.id}]`, inline: false });
    } else {
      fields.push({ name: "Kullanıcı", value: `${target.tag} [${target.id}]`, inline: false });
    }

      fields.push({ name: "Sebep", value: reason || "Hiçbir sebep belirtilmedi", inline: false });
    ...
    if (type.toUpperCase() === "MOVE") {
      fields.push({ name: "Kanala Taşındı =>", value: data.channel.name, inline: true });
    }
  }
  ...
  static async warnTarget(issuer, target, reason) {
    ...
    try {
      ...
      if (memberDb.warnings >= settings.max_warn.limit) {
          await ModUtils.addModAction(issuer.guild.members.me, target, "Maksimum uyarıya ulaşıldı", settings.max_warn.action); // moderate
  ...
  static async deafenTarget(issuer, target, reason) {
    ...
    try {
      ...
      error(`deafenTarget`, ex);
        return `sağır edilemedi ${target.user.tag}`;
    }
  }
