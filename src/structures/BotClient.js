...
module.exports = class BotClient extends Client {
  ...
  loadEvents(directory) {
      this.logger.log(`Etkinlikler yükleniyor...`);
    let success = 0;
    let failed = 0;
    const clientEvents = [];
    ...
    recursiveReadDirSync(directory).forEach((filePath) => {
      ...
      try {
        ...
        failed += 1;
          this.logger.error(`yük olayı - ${file}`, ex);
      }
    });
    ...
    console.log(
      ...
      this.logger.log(`${success + failed} olay yüklendi. Başarılı (${success}) Başarısız (${failed})`);
  }
  ...
  loadCommand(cmd) {
    ...
    if (cmd.category && CommandCategory[cmd.category]?.enabled === false) {
        this.logger.debug(`${cmd.name} Komut Atlanıyor. ${cmd.category} kategorisi devre dışı bırakıldı`);
      return;
    }
    // Prefix Command
    if (cmd.command?.enabled) {
      ...
      if (this.commandIndex.has(cmd.name)) {
          throw new Error(`Komut ${cmd.name} zaten kayıtlı`);
      }
      if (Array.isArray(cmd.command.aliases)) {
        cmd.command.aliases.forEach((alias) => {
            if (this.commandIndex.has(alias)) throw new Error(`${alias} takma adı zaten kayıtlı`);
      ...
      this.commands.push(cmd);
        ...
        this.logger.debug(`${cmd.name} komutu atlanıyor. Devre Dışı!`);
    }

    // Slash Command
    if (cmd.slashCommand?.enabled) {
        if (this.slashCommands.has(cmd.name)) throw new Error(`${cmd.name} Slash Komutu zaten kayıtlı`);
      this.slashCommands.set(cmd.name, cmd);
    } else {
        this.logger.debug(`${cmd.name} eğik çizgi komutu atlanıyor. Engelli!`);
    }
  }
  ...
  loadCommands(directory) {
    this.logger.log(`Komutlar Yükleniyor...`);
    ...
    for (const file of files) {
      ...
      this.logger.success(`${this.commands.length} komut yüklendi`);
      this.logger.success(`${this.slashCommands.size} slash komutları yüklendi`);
      if (this.slashCommands.size > 100) throw new Error("En fazla 100 slash komutu etkinleştirilebilir");
  }
  ...
  loadContexts(directory) {
      this.logger.log(`Bağlamlar yükleniyor...`);
    const files = recursiveReadDirSync(directory);
    for (const file of files) {
      try {
        ...
        validateContext(ctx);
          if (!ctx.enabled) return this.logger.debug(`${ctx.name} bağlamı atlanıyor. Engelli!`);
          if (this.contextMenus.has(ctx.name)) throw new Error(`Bağlam zaten bu ada sahip`);
    ...
    const messageContexts = this.contextMenus.filter((ctx) => ctx.type === "MESSAGE").size;
      ...
      if (userContexts > 3) throw new Error("En fazla 3 KULLANICI bağlamı etkinleştirilebilir");
      if (messageContexts > 3) throw new Error("En fazla 3 MESAJ bağlamı etkinleştirilebilir");

      this.logger.success(`${userContexts} USER içeriği yüklendi`);
      this.logger.success(`${messageContexts} MESSAGE bağlamı yüklendi`);
  }
  ...
  async registerInteractions(guildId) {
    ...
    // Register for a specific guild
      ...
      if (!guild) {
          this.logger.error(`Loncadaki etkileşimler kaydedilemedi ${guildId}`, new Error("Eşleşen lonca yok"));
        return;
      }
    ...
    // Throw an error
        ...
        throw new Error("Etkileşimleri kaydetmek için geçerli bir lonca kimliği sağladınız mı?");
    }

      this.logger.success("Başarıyla kaydedilen etkileşimler");
  }
